...
export function MyPurchases() {
  ...
  const filteredEscrows = escrows.filter(e => e.buyer === userDisplayAddress || e.buyer === 'You');

  return (
    <div className="space-y-6">
      <div className="mb-10">
        <h1 className="text-4xl font-bold mb-2 tracking-tight">My Purchases</h1>
        <p className="text-foreground/60 max-w-2xl">
          Transactions where you are the buyer and funds are currently locked in escrow.
        </p>
      </div>
        ...
        {filteredEscrows.length === 0 ? (
          <div className="bg-background p-20 rounded-3xl border border-primary/10 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-foreground/40">You haven't initiated any escrow transactions yet.</p>
            </div>
          </div>
        ) : (
